...
public final class Id3MetadataRetriever: MetadataRetrieverProtocol {
  ...
  init(_ metadataItems: [AVMetadataItem]) async {
    self.metadataItems = metadataItems

    await parseTrackNumber()
    parseDiscNumber()
  }

  private func parseTrackNumber() async {
    ...
    // Fallback to handle ID3v2 tags.
    if self.trackNumber == nil {
      let trackNumberItem = metadataItems.first { ($0.key as? String) == "TRK" }
      let trackNumberString = try? await trackNumberItem?.load(.stringValue)
      let trackNumber = trackNumberString == nil ? nil : Int(trackNumberString!)
      self.trackNumber = trackNumber
    }
  }
